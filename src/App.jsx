...
/* ============================================================
   ...
   The kit model, MIDI hook, metronome, and drum synth live in
   ./drumEngine.js — shared with the kids' Space Cadets app.
...
/* Stage-by-stage build illustrations, drawn in the app's realistic
   kit style. `step` (1–9) selects what's shown as the kit assembles. */
function AssemblyArt({ step }) {
  const FLOOR = 176;
  ...
  const Clamp = ({ x, y }) => <rect x={x - 5} y={y - 4} width="10" height="8" rx="2" fill={T.brass} stroke="#1A1712" strokeWidth="0.8" />;
  const Drum = ({ x, y, r = 20, glow }) => {
    const ry = r * 0.4, depth = r * 0.55, rim = glow ? T.strike : "#4C4636";
    return (
      <g>
        ...
        {glow && <ellipse cx={x} cy={y} rx={r + 4} ry={ry + 4} fill="none" stroke={T.strike} strokeWidth="2" opacity="0.7" />}
      </g>
    );
  };
  ...
  const Kick = ({ x, y, glow }) => (
    <g>
      ...
      <circle cx={x} cy={y} r="20" fill="url(#aKick)" stroke={glow ? T.strike : "#4C4636"} strokeWidth={glow ? 2.6 : 2} />
  ...
  const Module = ({ x, y, big }) => {
    ...
    return (
      <g>
        ...
        <rect x={x + w * 0.14} y={y + h * 0.22} width={w * 0.36} height={h * 0.28} rx="1" fill={T.brass} opacity="0.55" />
        <circle cx={x + w * 0.74} cy={y + h * 0.3} r={big ? 6 : 3.4} fill="#2E281F" stroke="#4A4232" strokeWidth="0.8" />
      </g>
    );
  };
  const Label = (x, y, t) => <text x={x} y={y} textAnchor="middle" fill={T.steel} style={{ font: `400 7px ${FONT_MONO}`, letterSpacing: "0.06em" }}>{t}</text>;
    ...
    <svg viewBox="0 0 320 200" style={{ width: "100%", height: "auto", display: "block" }}>
          ...
          {["SNARE", "TOM1", "TOM2", "TOM3", "HH", "CR", "RD", "KICK"].map((lbl, i) => {
            ...
            return (
              <g key={lbl}>
                ...
                <path d={`M ${jx},${jy + 3} C ${jx},${jy + 40} ${px},${py - 40} ${px},${py}`} fill="none" stroke={T.brass} strokeWidth="1.4" opacity="0.55" />
                <circle cx={px} cy={py} r="3.5" fill="none" stroke={T.brass} strokeWidth="1.4" />
                {Label(px, py + 13, lbl)}
              </g>
            );
          })}
      ...
      {step === 9 && (
        <g>
          ...
          {/* clamp bracket */}
          <rect x={150} y={92} width="34" height="36" rx="4" fill="#2A2620" stroke={T.brass} strokeWidth="2" />
          <rect x={156} y={98} width="22" height="10" rx="2" fill={T.brass} />
          {/* drum key turning */}
          <g transform="translate(167,150)">
            ...
            <path d="M 26 -18 A 26 26 0 0 1 14 6" fill="none" stroke={T.strike} strokeWidth="2.4" markerEnd="url(#arw)" />
          </g>
          <defs><marker id="arw" markerWidth="7" markerHeight="7" refX="3" refY="3" orient="auto"><path d="M0,0 L6,3 L0,6 Z" fill={T.strike} /></marker></defs>
        ...
        {ASM_STEPS.map((s) => (
          <Card key={s.icon}>
            <div style={{ display: "flex", gap: 14, alignItems: "flex-start" }}>
              ...
              <div style={{ flex: 1 }}>
                <h2 style={{ font: `700 18px ${FONT_DISPLAY}`, color: T.bone, margin: "4px 0 10px" }}>{s.t}</h2>
                <div style={{ borderRadius: 12, overflow: "hidden", border: `1px solid ${T.line}`,
                  background: "radial-gradient(circle at 50% 30%, #1E1A14, #15120E)", marginBottom: 12, padding: "6px 4px" }}>
    ...
    <div style={{ minHeight: "100vh", background: T.bg, color: T.bone, fontFamily: FONT_DISPLAY }}>
      ...
      <header style={{ position: "sticky", top: 0, zIndex: 10, background: "rgba(21,18,14,0.92)",
        ...
        <div style={{ maxWidth: 1080, margin: "0 auto", padding: "13px 22px",
          ...
          {onKidsMode && (
            <button onClick={onKidsMode} className="dc-focus" title="Switch to the kids' Space Cadets mode"
              style={{ font: `700 12px ${FONT_DISPLAY}`, color: T.bg, background: T.brass,
                border: "none", borderRadius: 999, padding: "7px 13px", cursor: "pointer", whiteSpace: "nowrap" }}>
              🚀 Kids
            </button>
          )}
