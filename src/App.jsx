...
const Card = ({ children, style, id }) => (
  <div id={id} style={{ background: T.bgCard, border: `1px solid ${T.line}`, borderRadius: 16, padding: 22, ...style }}>
    {children}
  </div>
);
  ...
  return (
    <div className="dc-rise" style={{ maxWidth: 760, margin: "0 auto" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        {ASM_STEPS.map((s) => (
          <Card key={s.icon} id={`asm-step-${s.icon}`} style={{ scrollMarginTop: 80 }}>
  ...
  },
  {
    ...
    sections: [
      ...
      { label: "הפזמון", subdiv: 2, bpm: 88,
        ...
        bars: [lanesFrom({ crash: [0], ride: [1, 2, 3, 4, 5, 6, 7], snare: [2, 6], kick: [0, 3, 4] })] },
      { label: "המעברון (פיל)", subdiv: 4, bpm: 84,
        teach: "המעברון שמחבר בין החלקים — גלגול יורד על הטומים בשש-עשרה: סנר, טום גבוה, טום אמצעי, טום רצפה. נגן אותו חלק ואחיד, ותנחת נקי בחזרה על הגרוב או על הפזמון.",
        bars: [lanesFrom({ snare: [0, 1, 2, 3], tom1: [4, 5, 6, 7], tom2: [8, 9, 10, 11], tom3: [12, 13, 14, 15] })] },
      { label: "לנגן את הכל", subdiv: 2, bpm: 86,
        teach: "המבנה המלא: שתי תיבות גרוב, מעברון קצר על הטומים, ואז קראש שנוחת על הפזמון. לחץ Demo כדי לשמוע את הכל, ואז נגן ברצף בלי לעצור.",
        bars: [
          lanesFrom({ hihat: R(8), snare: [2, 6], kick: [0, 4] }),
          lanesFrom({ hihat: R(8), snare: [2, 6], kick: [0, 4] }),
